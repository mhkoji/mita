(defpackage :mita.delivery.web.full.server-image
  (:use :cl)
  (:import-from :alexandria
                :when-let
                :when-let*)
  (:export :start))
(in-package :mita.delivery.web.full.server-image)
...
(defmethod mita.delivery.web.full.server::request-account-id ((req req))
  (req-account-id req))
   ...
   (lambda (req ent)
     (ensure-authenticated (account-id req :session-store session-store)
       (mita.web.app:image-serve
        (make-instance 'mita.delivery.web.full.server::spec
