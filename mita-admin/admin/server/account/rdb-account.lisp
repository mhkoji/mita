...
(defmethod mita.admin.account:find-account ((conn mita.rdb:connection)
                                            (username string))
  (when-let ((row (account-select conn username)))
    (mita.admin.account:construct-account
     :id (account-id row)
...
(defmethod mita.admin.account:find-account-by-id ((conn mita.rdb:connection)
...
(defmethod mita.admin.account:create-account ((conn mita.rdb:connection)
                                              (username string)
                                              (password string))
  (account-insert conn
                  (make-account
...
(defmethod mita.admin.account:delete-account ((conn mita.rdb:connection)
                                              (account-id mita.id:id))
  (account-delete conn account-id))

(defmethod load-accounts ((conn mita.rdb:connection))
  (mapcar (lambda (row)
            (mita.admin.account:construct-account
             :id (account-id row)
             :username (account-username row)))
