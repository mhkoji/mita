(defpackage :mita.rdb.common
  (:use :cl)
  (:import-from :mita.rdb
                :connection)
  (:export :insert-into
           :select-from
           :delete-from
...
(defmethod mita.rdb:image-select-by-ids ((conn connection)
                                         (image-id-list list))
  (mapcar #'parse-image
...
(defmethod mita.rdb:image-insert ((conn connection)
                                  (images list))
...
(defmethod mita.rdb:image-delete ((conn connection)
                                  (image-id-list list))
  (when image-id-list
...
(defmethod mita.rdb:album-delete ((conn connection)
                                  (album-id-list list))
  (when album-id-list
...
(defmethod mita.rdb:album-insert ((conn connection)
                                  (albums list))
  (insert-into conn "albums" '("album_id" "name" "created_on")
               (mapcar
                (lambda (album)
                  (list (mita.id:to-string (mita.rdb:album-id album))
                        (mita.rdb:album-name album)
                        (timestamp-to-string
                         conn (mita.rdb:album-created-on album))))
                albums)))

(defmethod mita.rdb:album-select ((conn connection)
                                  (album-id-list list))
  (when album-id-list
    (mapcar (lambda (row)
              (mita.rdb:make-album
...
(defmethod mita.rdb:album-thumbnail-image-delete ((conn connection)
                                                  (album-id-list list))
  (when album-id-list
...
(defmethod mita.rdb:album-thumbnail-image-select ((conn connection)
                                                  (album-id-list list))
  (when album-id-list
    (mapcar (lambda (row)
              (mita.rdb:make-album-thumbnail-image
...
(defmethod mita.rdb:album-thumbnail-image-insert ((conn connection)
                                                  (rows list))
  (insert-into conn "album_thumbnail_image" '("album_id" "image_id")
               (mapcar
                (lambda (row)
                  (list (mita.id:to-string
                         (mita.rdb:album-thumbnail-image-album-id row))
                        (mita.id:to-string
                         (mita.rdb:album-thumbnail-image-image-id row))))
                rows)))


(defmethod mita.rdb:album-image-insert ((conn connection)
                                        (album-id mita.id:id)
                                        (images list))
...
(defmethod mita.rdb:album-image-delete ((conn connection)
                                        (album-id-list list))
  (when album-id-list
...
(defmethod mita.rdb:album-image-select ((conn connection)
                                        (album-id mita.id:id))
  (mapcar #'parse-image
          (select-from conn
                       "i.image_id, i.path"
...
(defmethod mita.rdb:tag-delete ((conn connection)
                                (tag-id-list list))
  (when tag-id-list
...
(defmethod mita.rdb:tag-select ((conn connection))
  (mapcar #'parse-tag
          (select-from conn "*" "tags" :order-by "added_on")))

(defmethod mita.rdb:tag-insert ((conn connection)
                                (tag mita.tag:tag))
...
(defmethod mita.rdb:tag-content-delete ((conn connection)
                                        (tag-id mita.id:id))
  (delete-from conn "tag_content"
   :where `(:= "tag_id" (:p ,(mita.id:to-string tag-id)))))

(defmethod mita.rdb:tag-content-delete-by-content ((conn connection)
                                                   (content-id mita.id:id))
  (delete-from conn "tag_content"
   :where `(:= "content_id" (:p ,(mita.id:to-string content-id)))))

(defmethod mita.rdb:tag-content-select ((conn connection)
                                        (tag-id mita.id:id))
  (mapcar (lambda (row)
            (mita.rdb:make-content
             :id (mita.id:parse (first row))
...
(defmethod mita.rdb:tag-content-select-tags ((conn connection)
                                             (content-id mita.id:id))
  (mapcar #'parse-tag
          (select-from conn
                       "t.tag_id, t.name"
...
(defmethod mita.rdb:tag-content-insert ((conn connection)
                                        (tag-id mita.id:id)
                                        (contents list))
                 ...
                 (mapcar
                  (lambda (content)
                    (list (mita.id:to-string tag-id)
                          (string (mita.rdb:content-type content))
                          (mita.id:to-string (mita.rdb:content-id content))
                          added-on-string))
                  contents))))

(defmethod mita.rdb:tag-content-insert-by-tags ((conn connection)
                                                (tag-id-list list)
                                                (content mita.rdb:content))
  (let ((added-on-string (timestamp-to-string conn (local-time:now))))
    (insert-into conn "tag_content"
                 '("tag_id" "content_type" "content_id" "added_on")
                 (mapcar
